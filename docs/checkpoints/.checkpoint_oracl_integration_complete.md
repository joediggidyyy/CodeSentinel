...
### Placeholder Directory (Anchors for CLI navigation)

### PL-001 IncidentSyncAdapter abstraction

- **Status**: Scaffolded, pending activation

- **Scope**: Abstract outbound connector contract defined in `codesentinel/utils/integrations/incident_sync_adapter.py`.
- **Next Step**: Pair with first production adapter once Jira/ServiceNow approvals land.

### PL-002 NullIncidentSyncAdapter

- **Status**: Active placeholder

- **Scope**: Safe default adapter within `codesentinel/utils/integrations/null_incident_adapter.py` that logs intent only.
- **Next Step**: Retire when at least one production adapter is live.

### PL-003 Integration config toggles

- **Status**: Configured, disabled

- **Scope**: `codesentinel.json` integrations block + validation in `codesentinel/utils/config.py`.
- **Next Step**: Flip enable flags post-adapter QA; document in Change Control.

### PL-004 Ed25519 signer
