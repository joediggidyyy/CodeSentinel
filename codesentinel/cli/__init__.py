...
def main():
    ...
    if any(arg == "!!!!" for arg in sys.argv[1:]):
        ...
        sys.argv = [sys.argv[0]] + [
            "dev-audit" if a == "!!!!" else a for a in sys.argv[1:]
        ]
...
Examples:
  ...
  codesentinel !!!!                      # Quick trigger for dev-audit
    ...
    alert_parser.add_argument("--title", default="Manual Alert", help="Alert title")
    ...
    alert_parser.add_argument("--channels", nargs="+", help="Channels to send alert to")

    # Schedule command
    schedule_parser = subparsers.add_parser(
        "schedule", help="Manage maintenance scheduler"
    )
    schedule_parser.add_argument(
        "action", choices=["start", "stop", "status"], help="Scheduler action"
    )

    # Setup command
    setup_parser = subparsers.add_parser("setup", help="Run setup wizard")
    setup_parser.add_argument("--gui", action="store_true", help="Use GUI setup wizard")
    setup_parser.add_argument(
        "--non-interactive", action="store_true", help="Run non-interactive setup"
    )
    ...
    dev_audit_parser.add_argument(
        "--agent",
        action="store_true",
        help="Export audit context for AI agent remediation (requires GitHub Copilot)",
    )
    dev_audit_parser.add_argument(
    ...
    try:
        ...
        if args.command == "status":
            ...
            if args.output:
                ...
                print(
                    f"Scan completed. Found {results['summary']['total_vulnerabilities']} vulnerabilities."
                )
            ...
            if agent_mode:
                ...
                if export_path:
                    ...
                    print(
                        _json.dumps(
                            agent_context["remediation_context"]["summary"], indent=2
                        )
                    )
                    ...
                    print(
                        "An AI agent (GitHub Copilot) can now analyze these findings and build"
                    )
                    print(
                        "a remediation pipeline while respecting all persistent policies.\n"
                    )
                    ...
                    print(
                        "Remember: All actions must be non-destructive and preserve features."
                    )
            ...
            if interactive:
                ...
                print(
                    "A brief audit is running in the background; results will arrive via alerts."
                )
            else:
                import json as _json
