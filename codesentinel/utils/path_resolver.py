...
class PathResolver:
    ...
    def find_git_repositories(
        search_paths: List[Path], max_repos: int = 10
    ) -> List[dict]:
        """
    ...
    def get_common_dev_paths() -> List[Path]:
        ...
        if sys.platform == "win32":
            common_paths.extend(
                [Path("C:/Projects"), Path("D:/Projects"), home / "source"]
            )
        else:
            common_paths.extend([Path("/opt/projects"), home / "workspace"])
    ...
    def validate_codesentinel_structure(project_root: Path) -> dict:
        ...
        recommended_items = ["README.md", "requirements.txt", "launch.py"]
