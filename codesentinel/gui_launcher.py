...
class DependencyInstallerGUI:
    ...
    def setup_ui(self):
        ...
        title_label = ttk.Label(
            header_frame, text="🚀 CodeSentinel Setup", font=("Segoe UI", 16, "bold")
        )
        title_label.pack()
        ...
        self.install_button = ttk.Button(
            button_frame, text="Install Dependencies", command=self.start_installation
        )
        ...
        self.close_button = ttk.Button(
            button_frame, text="Close", command=self.root.destroy, state="disabled"
        )
    ...
    def install_dependencies(self):
        ...
        try:
            ...
            self.update_status(
                f"Installing {len(missing_packages)} missing packages..."
            )
            self.log_message(
            ...
            for i, package in enumerate(missing_packages, 1):
                self.update_status(
                    f"Installing {package} ({i}/{len(missing_packages)})..."
                )
                self.log_message(f"\n🔧 Installing {package}...")
