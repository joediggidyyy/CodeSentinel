...
class WizardApp:
    ...
    def __init__(self):
        ...
        # Validation states
        self.validations = {"email": False, "slack": False, "github": False}
    ...
    def _build_ui(self):
        ...
        ttk.Label(
            self.header, text="CodeSentinel Setup Wizard", style="Title.TLabel"
        ).pack()
        ttk.Label(
            self.header,
        ...
        self.progress_bar = ttk.Progressbar(
            progress_frame, variable=self.progress_var, maximum=8
        )
        ...
        self.cancel_btn = ttk.Button(
            self.footer, text="Cancel", command=self.root.destroy
        )
    ...
    def _update_nav_state(self):
        ...
        if self.current == 1:  # Project Setup step (GitHub)
            ...
            github_needs_validation = (
                hasattr(self, "github_enabled_var")
                and self.github_enabled_var.get()
                and not self.validations.get("github", False)
            )
            ...
            email_needs_validation = (
                hasattr(self, "email_var")
                and self.email_var.get()
                and not self.validations.get("email", False)
            )
            slack_needs_validation = (
                hasattr(self, "slack_var")
                and self.slack_var.get()
                and not self.validations.get("slack", False)
            )
    ...
    def _step_welcome(self):
        ...
        ttk.Label(
            f,
            text="Welcome to CodeSentinel! This wizard will guide you through project setup and configuration.",
            font=("Arial", 9),
            justify="left",
            foreground="#333333",
            wraplength=850,
        ...
        progress_frame = ttk.LabelFrame(
            left_column, text="⚙️ Installation Progress", padding=10
        )
        ...
        provides_frame = ttk.LabelFrame(
            left_column, text="🛡️ What CodeSentinel Provides", padding=10
        )
        ...
        env_frame = ttk.LabelFrame(
            right_column, text="📂 Environment Information", padding=10
        )
        ...
        for label, value in env_items:
            ...
            ttk.Label(item_frame, text=label, font=("Arial", 9, "bold"), width=18).pack(
                side="left", anchor="w"
            )
        ...
        start_frame = ttk.LabelFrame(
            right_column, text="🚀 Getting Started", padding=10
        )
    ...
    def _step_location(self):
        ...
        ttk.Label(
            f,
            text="Configure your installation location and GitHub integration.",
            font=("Arial", 9),
            foreground="gray",
        ).pack(anchor="w", pady=(0, 10))
        ...
        location_frame = ttk.LabelFrame(
            left_column, text="📂 Installation Location", padding=10
        )
        ...
        ttk.Label(
            location_frame, text="Install location (project root):", font=("Arial", 9)
        ).pack(anchor="w", pady=(0, 4))

        entry_row = ttk.Frame(location_frame)
        entry_row.pack(fill="x", pady=(0, 8))
        ttk.Entry(entry_row, textvariable=self.loc_var, font=("Arial", 9)).pack(
            side="left", fill="x", expand=True
        )
        ttk.Button(entry_row, text="Browse...", command=self._browse_location).pack(
            side="left", padx=(8, 0)
        )
        ...
        if detected_repos:
            ...
            ttk.Button(
                location_frame,
                text="Use Selected Repository",
                command=self._use_selected_repo,
            ).pack(anchor="e")
        ...
        github_frame = ttk.LabelFrame(
            right_column, text="🔗 GitHub Integration", padding=10
        )
        github_frame.pack(fill="both", expand=True)

        # Enable GitHub checkbox
        self.github_enabled_var = tk.BooleanVar(
            value=self.data["github"].get("enabled", False)
        )
        ttk.Checkbutton(
            github_frame,
            text="✓ Enable GitHub Integration",
            variable=self.github_enabled_var,
            command=self._on_github_toggle,
        ...
        for value, label in modes:
            ttk.Radiobutton(
                self.github_config_frame, text=label, variable=self.gh_mode, value=value
            ).pack(anchor="w", pady=2)

        # Repository URL
        ttk.Label(
            self.github_config_frame, text="Repository URL:", font=("Arial", 9)
        ).pack(anchor="w", pady=(8, 4))
        self.gh_url = tk.StringVar(value=self.data["github"].get("repo_url", ""))
        ttk.Entry(
            self.github_config_frame, textvariable=self.gh_url, font=("Arial", 9)
        ).pack(fill="x", pady=(0, 2))
        ttk.Label(
            self.github_config_frame,
            text="Example: https://github.com/username/repo",
            font=("Arial", 8),
            foreground="gray",
        ...
        ttk.Button(
            validation_frame, text="🔍 Validate", command=self._validate_github
        ).pack(side="left")
        ...
        def collect():
            self.data["install_location"] = self.loc_var.get().strip() or str(
                Path.cwd()
            )
    ...
    def _step_alerts(self):
        ...
        ttk.Checkbutton(
            channels, text="✓ Console Output", variable=self.console_var
        ...
        filebox = ttk.LabelFrame(
            left_column, text="📁 File Logging Configuration", padding=12
        )
        filebox.pack(fill="x", pady=(0, 12))
        self.log_file_var = tk.StringVar(value=alerts["file"]["log_file"])
        ttk.Label(filebox, text="Log file path:", font=("Arial", 9)).pack(
            anchor="w", pady=(0, 4)
        )
        ttk.Entry(filebox, textvariable=self.log_file_var, font=("Arial", 9)).pack(
            fill="x"
        )

        # Email Configuration
        email = ttk.LabelFrame(
            left_column, text="📧 Email Alert Configuration", padding=12
        )
        email.pack(fill="both", expand=True)

        self.smtp_server_var = tk.StringVar(
            value=alerts["email"].get("smtp_server", "smtp.gmail.com")
        )
        self.smtp_port_var = tk.StringVar(
            value=str(alerts["email"].get("smtp_port", 587))
        )
        ...
        self.to_emails_var = tk.StringVar(
            value=",".join(alerts["email"].get("to_emails", []))
        )
        ...
        for lbl, var, is_password in email_fields:
            ...
            ttk.Label(row_frame, text=lbl, font=("Arial", 9), width=20).pack(
                side="left", anchor="w"
            )
            ttk.Entry(
                row_frame,
        ...
        ttk.Button(
            email_val_frame, text="🔍 Test Configuration", command=self._validate_email
        ).pack(side="left")
        ...
        # Slack Configuration
        slack = ttk.LabelFrame(
            right_column, text="💬 Slack Integration Configuration", padding=12
        )
        slack.pack(fill="x")

        self.slack_url_var = tk.StringVar(value=alerts["slack"].get("webhook_url", ""))
        self.slack_channel_var = tk.StringVar(
            value=alerts["slack"].get("channel", "#maintenance-alerts")
        )

        # Webhook URL
        url_frame = ttk.Frame(slack)
        url_frame.pack(fill="x", pady=(0, 8))
        ttk.Label(url_frame, text="Webhook URL:", font=("Arial", 9)).pack(
            anchor="w", pady=(0, 4)
        )
        ttk.Entry(url_frame, textvariable=self.slack_url_var, font=("Arial", 9)).pack(
            fill="x"
        )
        ...
        ttk.Label(channel_frame, text="Channel:", font=("Arial", 9)).pack(
            anchor="w", pady=(0, 4)
        )
        ttk.Entry(
            channel_frame, textvariable=self.slack_channel_var, font=("Arial", 9)
        ).pack(fill="x")
        ...
        ttk.Button(
            slack_val_frame, text="🔍 Test Connection", command=self._validate_slack
        ).pack(side="left")
        ...
        def collect():
            ...
            alerts["file"]["log_file"] = (
                self.log_file_var.get().strip() or "codesentinel.log"
            )
            ...
            alerts["email"].update(
                {
                    ...
                    "to_emails": [
                        e.strip()
                        for e in self.to_emails_var.get().split(",")
                        if e.strip()
                    ],
                }
            )
            ...
            alerts["slack"].update(
                {
                    ...
                    "channel": self.slack_channel_var.get().strip()
    ...
    def _validate_email(self):
        ...
        self.email_status.config(
            text="🔍 Testing...", foreground=self.colors["processing"]
        )
        self.root.update()
        ...
        try:
            ...
            if not all([server, username, password, from_addr]):
                self.email_status.config(
                    text="❌ Missing required fields", foreground=self.colors["error"]
                )
            ...
            self.email_status.config(
                text="✓ Configuration valid", foreground=self.colors["success"]
            )
            self.validations["email"] = True
            self._update_nav_state()
        except Exception as e:
            self.email_status.config(
                text=f"❌ Error: {str(e)[:30]}...", foreground=self.colors["error"]
            )
            self.validations["email"] = False
            self._update_nav_state()
    ...
    def _validate_slack(self):
        ...
        try:
            ...
            if not webhook_url:
                self.slack_status.config(
                    text="❌ Webhook URL required", foreground=self.colors["error"]
                )
            ...
            if response.status == 200:
                self.slack_status.config(
                    text="✓ Webhook valid", foreground=self.colors["success"]
                )
                self.validations["slack"] = True
                self._update_nav_state()
            else:
                self.slack_status.config(
                    text=f"❌ HTTP {response.status}", foreground=self.colors["error"]
                )
                self.validations["slack"] = False
                self._update_nav_state()
        except Exception as e:
    ...
    def _on_email_toggle(self):
        ...
        if self.email_var.get():
            ...
            self.validations["email"] = False
            self.email_status.config(
                text="⚠️ Not validated", foreground=self.colors["warning"]
            )
        else:
    ...
    def _set_widget_state_recursive(self, widget, state):
        ...
        try:
            if isinstance(
                widget, (ttk.Button, ttk.Radiobutton, ttk.Entry, ttk.Checkbutton)
            ):
    ...
    def _validate_github(self):
        ...
        self.github_status.config(
            text="🔍 Validating...", foreground=self.colors["processing"]
        )
        self.root.update()
            ...
            if len(parts) >= 2:
                ...
                try:
                    ...
                    if e.code == 404:
                        ...
                        self.github_status.config(
                            text=f"❌ HTTP {e.code}", foreground=self.colors["error"]
                        )
                    self.validations["github"] = False
            else:
                self.github_status.config(
                    text="❌ Invalid URL format", foreground=self.colors["error"]
                )
                self.validations["github"] = False
    ...
    def _step_ide(self):
        ...
        ttk.Label(
            f,
            text="Configure integration with your development environment.",
            font=("Arial", 9),
            foreground="gray",
        ).pack(anchor="w", pady=(0, 8))
        ...
        for config in ide_configs:
            ...
            if not found:
                ...
                ttk.Button(ide_row, text="Download", command=open_url).pack(
        ...
        def collect():
            ...
            for key, var in self.ide_vars.items():
                ide_data[key] = {"detected": statuses[key], "enabled": var.get()}
    ...
    def _step_copilot(self):
        ...
        ttk.Label(
            f,
            text="Configure AI-powered code assistance and intelligent automation.",
            font=("Arial", 9),
            foreground="gray",
        ).pack(anchor="w", pady=(0, 10))
        ...
        detection_frame = ttk.LabelFrame(
            left_column, text="🔍 IDE Detection", padding=10
        )
        ...
        if copilot_detected:
            ttk.Label(
                detection_frame,
                text="✓ VS Code detected - Copilot integration available",
                font=("Arial", 9),
                foreground=self.colors["success"],
            ).pack(anchor="w")
        else:
            ttk.Label(
                detection_frame,
                text="⚠ VS Code not detected - Install for full integration",
                font=("Arial", 9),
        ...
        options_frame = ttk.LabelFrame(
            left_column, text="⚙️ Integration Options", padding=10
        )
        options_frame.pack(fill="both", expand=True)

        self.copilot_enabled = tk.BooleanVar(value=self.data["copilot"]["enabled"])
        self.copilot_vscode_ext = tk.BooleanVar(
            value=self.data["copilot"]["install_vscode_extension"]
        )
        self.copilot_instructions = tk.BooleanVar(
            value=self.data["copilot"]["generate_instructions"]
        )
        self.copilot_agent_mode = tk.BooleanVar(
            value=self.data["copilot"]["enable_agent_mode"]
        )

        # Main enable checkbox
        ttk.Checkbutton(
            options_frame,
            text="✓ Enable GitHub Copilot Integration",
            variable=self.copilot_enabled,
            command=self._toggle_copilot_options,
        ).pack(anchor="w", pady=(0, 6))
        ...
        ttk.Checkbutton(
            self.copilot_sub_frame,
            text="📝 Generate Copilot Instructions",
        ...
        # Benefits section
        benefits_frame = ttk.LabelFrame(
            right_column, text="✨ What You Get", padding=10
        )
        ...
        for benefit in benefits:
            ttk.Label(
                benefits_frame, text=benefit, font=("Arial", 9), foreground="#424242"
            ).pack(anchor="w", pady=2)

        # Installation notes
        note_frame = ttk.LabelFrame(
            right_column, text="ℹ️ Installation Notes", padding=10
        )
        ...
        for note in notes:
            ttk.Label(note_frame, text=note, font=("Arial", 8), foreground="gray").pack(
                anchor="w", pady=1
            )
    ...
    def _step_optional(self):
        ...
        ttk.Label(
            f,
            text="Configure additional automation features that enhance security and workflow.",
            font=("Arial", 9),
            foreground="gray",
        ).pack(anchor="w", pady=(0, 10))
        ...
        features_frame = ttk.LabelFrame(
            f, text="🚀 Automation & Integration Features", padding=10
        )
        ...
        recommendations_frame = ttk.LabelFrame(
            f, text="💡 Smart Recommendations", padding=10
        )
        ...
        try:
            if (Path.cwd() / ".git").exists():
                recommendations.append(
                    "Git Hooks highly recommended for your Git repository"
                )
            if self.data.get("github", {}).get("enabled", False):
                recommendations.append(
                    "CI/CD templates complement your GitHub integration"
                )
            if not recommendations:
                recommendations.append(
                    "Git Hooks provide immediate value with minimal setup"
                )
        except:
            recommendations.append(
                "Review each feature to choose what fits your workflow"
            )
    ...
    def _create_optional_feature(
        ...
        ttk.Label(
            ...
            text=f"📊 {complexity}",
            font=("Arial", 8),
            foreground=complexity_colors.get(complexity, "black"),
        ).pack(side="right", padx=(8, 0))

        ttk.Label(
            badge_frame, text=f"🎯 {impact}", font=("Arial", 8), foreground="#424242"
        ).pack(side="right", padx=(8, 0))

        # Subtitle
        ttk.Label(
            feature_frame, text=subtitle, font=("Arial", 9, "italic"), foreground="gray"
        ).pack(anchor="w", pady=(1, 3), padx=(20, 0))
        ...
        toggle_btn = ttk.Button(
            desc_frame, text="ℹ️ Show Details", command=toggle_description
        )
    ...
    def _step_summary(self):
        ...
        ttk.Label(
            f,
            text="Review your configuration, then click Finish to save.",
            font=("Arial", 9),
            foreground="gray",
        ).pack(anchor="w", pady=(0, 10))
        ...
        def collect():
            ...
            try:
                ...
                summary.append("╔" + "═" * 58 + "╗")
                summary.append(
                    "║" + "  CODESENTINEL CONFIGURATION SUMMARY".center(58) + "║"
                )
                summary.append("╚" + "═" * 58 + "╝")
                summary.append("")
                ...
                summary.append("   ─────────────────────────────────────────────")
                summary.append(
                    f"   Location: {self.data.get('install_location', 'Not set')}"
                )
                github_data = self.data.get("github", {})
                summary.append(
                    f"   Mode:     {github_data.get('mode', 'Not set').upper()}"
                )
                summary.append("")
                ...
                if file_alerts.get("enabled"):
                    ...
                    summary.append(
                        f"      • Path:  {file_alerts.get('log_file', 'codesentinel.log')}"
                    )
                else:
                ...
                if email_alerts.get("enabled"):
                    ...
                    summary.append(
                        f"      • To:   {', '.join(to_emails) if to_emails else 'Not set'}"
                    )
                    summary.append(
                        f"      • SMTP: {email_alerts.get('smtp_server', '')}:{email_alerts.get('smtp_port', 587)}"
                    )
                else:
                ...
                if slack_alerts.get("enabled"):
                    ...
                    summary.append(
                        f"      • Channel: {slack_alerts.get('channel', '#maintenance-alerts')}"
                    )
                else:
                ...
                summary.append("─" * 60)
                summary.append(
                    "✓ Configuration complete - Click 'Finish' to save and exit"
                )
                summary.append("─" * 60)
                ...
                self.summary_text.insert(
                    "1.0", f"Error generating summary:\n{str(e)}\n\nData: {self.data}"
                )
    ...
    def _browse_location(self):
        selected = filedialog.askdirectory(
            initialdir=self.loc_var.get() or str(Path.home())
        )
        if selected:
        ...
        if self.data.get("github", {}).get("mode") == "initialize":
            if not (install_path / ".git").exists():
                try:
                    ...
                    subprocess.run(
                        ["git", "init"], cwd=str(install_path), capture_output=True
                    )
                except Exception:
                    pass
        ...
        if config.get("install_vscode_extension", False):
            try:
                ...
                if shutil.which("code"):
                    ...
                    results.append(
                        "✓ Created .vscode/settings.json with CodeSentinel config"
                    )
                else:
