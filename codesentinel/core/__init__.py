...
class CodeSentinel:
    ...
    def __init__(self, config_path: Optional[Path] = None):
        ...
        try:
            check_interval = self.config.get("process_monitor", {}).get(
                "check_interval", 60
            )
            enabled = self.config.get("process_monitor", {}).get("enabled", True)
            self.process_monitor = start_monitor(
                check_interval=check_interval, enabled=enabled
            )
    ...
    def run_security_scan(self) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                subprocess.run(
                    [sys.executable, "-c", 'print("codesentinel security scan")'],
                    capture_output=True,
                    text=True,
                )
            except Exception:
            ...
            results["summary"] = {
                ...
                "scan_duration": (
                    datetime.now() - datetime.fromisoformat(results["timestamp"])
                ).total_seconds(),
            }
    ...
    def run_maintenance_tasks(self, task_type: str = "daily") -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                subprocess.run(
                    [sys.executable, "-c", 'print("codesentinel maintenance")'],
                    capture_output=True,
                    text=True,
                )
            except Exception:
            ...
            self.logger.info(
                f"{task_type.capitalize()} maintenance completed successfully."
            )
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "alert_channels": list(
                self.config.get("alerts", {}).get("channels", {}).keys()
            ),
